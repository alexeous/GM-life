...
void assert_data(const unsigned char* exp, size_t expsize,
    ...
    size_t i;
    if (expsize != realsize) {
        CTEST_ERR("%s:%d  expected %lu bytes, got %lu", caller, line, (uintmax_t) expsize, (uintmax_t) realsize);
    }
    for (i=0; i<expsize; i++) {
        if (exp[i] != real[i]) {
            CTEST_ERR("%s:%d expected 0x%02x at offset %lu got 0x%02x",
...
void assert_equal_u(uintmax_t exp, uintmax_t real, const char* caller, int line) {
    if (exp != real) {
        CTEST_ERR("%s:%d  expected %lu, got %lu", caller, line, exp, real);
    }
}
...
void assert_not_equal_u(uintmax_t exp, uintmax_t real, const char* caller, int line) {
    if ((exp) == (real)) {
        CTEST_ERR("%s:%d  should not be %lu", caller, line, real);
    }
}

void assert_interval(intmax_t exp1, intmax_t exp2, intmax_t real, const char* caller, int line) {
    if (real < exp1 || real > exp2) {
        CTEST_ERR("%s:%d  expected %ld-%ld, got %ld", caller, line, exp1, exp2, real);
    }
}
...
int ctest_main(int argc, const char *argv[])
{
    ...
    char results[80];
    sprintf(results, "RESULTS: %d tests (%d ok, %d failed, %d skipped) ran in %lu ms", total, num_ok, num_fail, num_skip, (t2 - t1)/1000);
